  ...
  mutate () {
    ...
    if (rand_in_range(0, 2) === 0) {
      ...
      for (let i = 0; i < this.color.length; i++) {
        let change = rand_in_range(-10, 10)
    ...
    // 1 in 2 chance to wiggle a point
      ...
      let new_x = this.points[index] + rand_in_range(-10, 10)
      let new_y = this.points[index+1] + rand_in_range(-10, 10)

      this.points[index] = put_in_range(new_x, 0, this.width)
      this.points[index+1] = put_in_range(new_y, 0, this.height)
    }

    // 1 in 10 chance to add a new point
...
class Individual {
  ...
  attemptMutation () {
    ...
    // 1 in 1000 chance to either gain or lose a gene
  ...
  fitness (goal) {
    ...
    let max_diff = Math.pow(255, 3) * this.width * this.height
    this.fitness = (diff / max_diff)

    // this.fitness = 100 * (diff / (256 * 256 * 256 * this.width * this.height))
    return this.fitness
  }
